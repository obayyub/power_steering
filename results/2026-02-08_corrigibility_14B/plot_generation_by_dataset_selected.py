#!/usr/bin/env python3
"""Plot % corrigible by dataset for selected best vectors."""
...
def main():
    ...
    fig.suptitle('% Corrigible by Dataset (Qwen3-14B, temp=0.7)', fontsize=13, fontweight='bold')

    for ax, ds in zip(axes, datasets):
        for i, (label, vec) in enumerate(selected.items()):
            pcts = []
            for scale in scales:
                s = stats.get((vec, scale, ds), {'corrigible': 0, 'total': 0})
    ...
    plt.tight_layout()
    out_path = out_dir / "generation_by_dataset_selected.png"
